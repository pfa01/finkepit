...
class PaymentAnonymizer:
    ...
    def __init__(self, config_path: str):
        ...
        self._reference_ibans: set = None   # lazy-loaded
    ...
    def _ensure_directories(self):
        ...
        # Routing-Ordner nur anlegen wenn Referenz-CSV konfiguriert ist
        if self.config.iban_reference_csv:
            dirs['available_path']     = self.config.available_path
            dirs['not_available_path'] = self.config.not_available_path
    ...
    # -------------------------------------------------------------------------
    # IBAN-Routing: available / not_available
    # -------------------------------------------------------------------------

    @property
    def reference_ibans(self) -> set:
        """
        Gibt die Menge der Referenz-IBANs zurueck (lazy-loaded, gecacht).
        Leer wenn iban_reference_csv nicht konfiguriert oder nicht gefunden.
        """
        if self._reference_ibans is None:
            self._reference_ibans = self.config.load_reference_ibans()
        return self._reference_ibans

    _IBAN_RE = re.compile(r'\b[A-Z]{2}\d{2}[A-Z0-9]{4,30}\b')

    def _extract_ibans_from_content(self, content: str) -> set:
        """Extrahiert alle IBAN-artigen Zeichenketten aus dem Rohinhalt."""
        return set(self._IBAN_RE.findall(content))

    def _route_to_availability_folder(self,
                                       original_content: str,
                                       output_path: Path) -> Path:
        """
        Prueft ob die Originaldatei eine IBAN enthaelt die in der
        Referenz-CSV steht und verschiebt die anonymisierte Output-Datei
        in den passenden Unterordner:

          output/available/      – mindestens eine IBAN in der Referenzliste
          output/not_available/  – keine IBAN in der Referenzliste
    ...
    def process_file(self, input_path: Path) -> AnonymizationResult:
        ...
        try:
            ...
            # ── IBAN-Routing: available / not_available ────────────────────
            final_path          = self._route_to_availability_folder(
                content, output_path
            )
