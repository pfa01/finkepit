...
class ISO20022Anonymizer(BaseAnonymizer):
    ...
    def anonymize(self, content: str) -> Tuple[str, int]:
        ...
        try:
            ...
            # Schritt 2a: Instruktions- und Regulatory-Felder
            self.fields_anonymized += self._anonymize_instruction_fields(
                root, processed_ids
            )
    ...
    # -------------------------------------------------------------------------
    # Instruktions- und Regulatory-Felder
    # -------------------------------------------------------------------------

    def _anonymize_instruction_fields(self, root: etree._Element,
                                       processed_ids: set) -> int:
        """
        Anonymisiert Freitext-Felder in Instruktions- und
        Regulatory-Reporting-Elementen.

        Behandelte Felder:
          InstrForCdtrAgt/InstrInf  – Instruktion an Glaeubiger-Agent
                                      (pacs.008, pacs.009, pain.001)
          InstrForNxtAgt/InstrInf   – Instruktion an naechsten Agenten
                                      (pacs.008, pacs.009)
          InstrForDbtrAgt/InstrInf  – Instruktion an Schuldner-Agenten
                                      (pain.001)
          RgltryRptg/Dtls/Inf       – Freitext im regulatorischen Reporting
          StsRsnInf/AddtlInf        – Zusatzinfo Statusgrund (pacs.002)
          Nrtv                      – Narrativ-Freitext (diverse)

        Alle InstrInf-Felder werden mit einem einzigen XPath-Aufruf ueber
        das gesamte Dokument erfasst – unabhaengig vom Eltern-Element.
        """
        count = 0

        # ── InstrInf – alle Vorkommen im Dokument ────────────────────────
        if self.config.anonymize_instruction_fields:
            if not self.remittance_anonymizer.is_enabled:
                logger.debug(
                    "InstrInf-Anonymisierung uebersprungen "
                    "(remittance_anonymizer deaktiviert)."
                )
            else:
                for elem in root.xpath(".//*[local-name()='InstrInf']"):
                    ...
                    parent    = (
                        etree.QName(elem.getparent().tag).localname
                        if elem.getparent() is not None else '?'
                    )
                    new_value = self.remittance_anonymizer.anonymize(original)
                    logger.debug(
                        "[INSTR_INF] parent=<%s>  alt=%s   neu=%s",
                        parent, original[:40], new_value[:40]
                    )
                    elem.text = new_value
                    processed_ids.add(id(elem))
                    count += 1
        ...
        # ── RgltryRptg/Dtls/Inf ──────────────────────────────────────────
        if self.config.anonymize_regulatory_reporting:
            if not self.remittance_anonymizer.is_enabled:
                logger.debug(
                    "RgltryRptg-Anonymisierung uebersprungen "
                    "(remittance_anonymizer deaktiviert)."
                )
            else:
                for rgltry in root.xpath(".//*[local-name()='RgltryRptg']"):
                    for dtls in rgltry.xpath(".//*[local-name()='Dtls']"):
                        for inf in dtls.xpath(".//*[local-name()='Inf']"):
                            if id(inf) in processed_ids:
                                continue
                            if not inf.text or not inf.text.strip():
                                continue
                            original  = inf.text.strip()
                            new_value = self.remittance_anonymizer.anonymize(
                                original
                            )
                            logger.debug(
                                "[RGLTRY_INF] alt=%s   neu=%s",
                                original[:40], new_value[:40]
                            )
                            inf.text = new_value
                            processed_ids.add(id(inf))
                            count += 1

        # ── StsRsnInf/AddtlInf ───────────────────────────────────────────
        if self.config.anonymize_instruction_fields:
            if self.remittance_anonymizer.is_enabled:
                for sts in root.xpath(".//*[local-name()='StsRsnInf']"):
                    for addtl in sts.xpath(".//*[local-name()='AddtlInf']"):
                        if id(addtl) in processed_ids:
                            continue
                        if not addtl.text or not addtl.text.strip():
                            continue
                        original  = addtl.text.strip()
                        new_value = self.remittance_anonymizer.anonymize(original)
                        logger.debug(
                            "[STS_RSN_ADDTL] alt=%s   neu=%s",
                            original[:40], new_value[:40]
                        )
                        addtl.text = new_value
                        processed_ids.add(id(addtl))
                        count += 1

        # ── Nrtv – Narrativ-Freitext ─────────────────────────────────────
        if self.config.anonymize_instruction_fields:
            if self.remittance_anonymizer.is_enabled:
                for elem in root.xpath(".//*[local-name()='Nrtv']"):
                    if id(elem) in processed_ids:
                        continue
                    if not elem.text or not elem.text.strip():
                        continue
